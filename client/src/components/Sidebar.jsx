...
export const Sidebar = ({ 
  ...
  // Grouped Navigation matching specification
  const navSections = [
    {
      title: 'Dashboard',
      items: [
        { id: 'dashboard', label: 'Admin Dashboard', icon: LayoutDashboard, badge: null },
        { id: 'employee-dashboard', label: 'Employee Dashboard', icon: UserCheck, badge: null },
      ]
    },
    {
      title: 'Security',
      items: [
        { id: 'risk-analysis', label: 'Risk Analysis', icon: Activity, badge: null },
        { id: 'xai', label: 'Explainable AI', icon: Sparkles, badge: 'XAI' },
        { id: 'devices', label: 'Device Trust', icon: Laptop, badge: null },
        { id: 'incidents', label: 'Alerts / Incidents', icon: ShieldAlert, badge: incidentCount > 0 ? incidentCount : null },
        { id: 'policies', label: 'Policy Engine', icon: Shield, badge: null },
      ]
    },
    {
      title: 'Monitoring',
      items: [
        ...
        { id: 'employees', label: 'Employee Management', icon: Users, badge: null },
        { id: 'departments', label: 'Departments', icon: Building2, badge: null },
        { id: 'ml-analytics', label: 'ML Analytics', icon: BrainCircuit, badge: null },
        { id: 'simulator', label: 'Threat Simulator', icon: FlaskConical, badge: 'SIM' },
      ]
    },
    {
      title: 'System',
  ...
  const sidebarContent = (
    <aside 
      ...
      className={`h-full border-r transition-all duration-300 flex flex-col z-30 shrink-0 select-none ${
        isDark ? 'bg-[#0E0E0E] border-[#2A2A2A] text-white' : 'bg-[#FFFFFF] border-[#E5E5E5] text-[#0E0E0E]'
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-4 custom-scrollbar">
        {navSections.map((section, sIdx) => (
          <div key={sIdx} className="space-y-1">
            {!isCollapsed ? (
              <div className={`text-[10px] font-mono px-3 pt-2 pb-1 uppercase tracking-[0.15em] font-medium ${
                isDark ? 'text-[#6B6B6B]' : 'text-[#6B6B6B]'
              }`}>
                {section.title}
              </div>
            ) : (
              <div className="text-center text-[9px] text-[#6B6B6B] my-1">•••</div>
            )}

            {section.items.map((item) => {
              const Icon = item.icon;
              const isActive = activeTab === item.id || 
                (item.id === 'employees' && (activeTab === 'users' || activeTab === 'employee-mgmt')) ||
                (item.id === 'risk-analysis' && activeTab === 'ai-risk');
              ...
              return (
                ...
                >
                  <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-[#C6A14A]' : 'opacity-70'}`} />
                  ...
                  {!isCollapsed && item.badge !== null && (
                    <span className={`text-[8px] font-mono px-1.5 py-0.5 uppercase tracking-wider font-semibold border rounded-xs ${
                      isActive
      ...
      <div className={`p-3 border-t space-y-2 ${isDark ? 'border-[#2A2A2A]' : 'border-[#E5E5E5]'}`}>
        {!isCollapsed && (
          <div className={`p-2.5 border rounded-sm flex items-center justify-between text-xs ${
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0 truncate">
                <div className="font-semibold text-[11px] truncate leading-tight">{adminName}</div>
        ...
        <button
          ...
          className={`hidden md:flex w-full items-center justify-center gap-2 p-1.5 text-xs transition-colors cursor-pointer ${
            isDark ? 'text-gray-400 hover:text-[#C6A14A]' : 'text-gray-600 hover:text-[#C6A14A]'
          }`}
        >
          <ChevronRight className={`w-3.5 h-3.5 transition-transform duration-200 ${isCollapsed ? '' : 'rotate-180'}`} />
          {!isCollapsed && <span className="text-[11px] font-mono uppercase tracking-wider">Collapse Sidebar</span>}
        </button>
      </div>
    </aside>
  );

  // Render desktop sidebar + mobile drawer
  return (
    <>
      {/* Desktop Sidebar */}
