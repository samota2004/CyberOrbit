...
export const DepartmentsPage = ({
  ...
  const getDeptStats = (dept) => {
    ...
    const deptUsers = safeUsers.filter((user) => {
      const userDept = String(user?.department || '')
        .trim()
        .toUpperCase();

      return userDept === normalizedDept;
    });

    const totalUsers = deptUsers.length;

    const highRiskUsers = deptUsers.filter((user) => {
      const risk = Number(user?.currentRiskScore ?? 0);
      return risk >= 60;
    }).length;

    const activeUsers = deptUsers.filter((user) => {
    ...
    const avgRisk =
      totalUsers > 0
        ? Math.round(
            deptUsers.reduce(
              (total, user) =>
                total + Number(user?.currentRiskScore ?? 0),
              0
            ) / totalUsers
          )
        : 0;
    ...
    const avgTrust =
      totalUsers > 0
        ? Math.round(
            deptUsers.reduce(
              (total, user) =>
                total + Number(user?.currentTrustScore ?? 0),
              0
            ) / totalUsers
          )
        : 0;

    const deptIncidents = safeIncidents.filter((incident) => {
      const incidentDept = String(
      ...
      return incidentDept === normalizedDept;
    }).length;
  ...
  const selectedDeptStats = selectedDept
    ? getDeptStats(selectedDept)
    : null;
    ...
    <div className="space-y-10">
      ...
      {selectedDept && selectedDeptStats && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs">
          <div
            className={`w-full max-w-lg h-full border-l border-[#D4AF37]/30 p-8 overflow-y-auto space-y-6 shadow-2xl ${
              isDark
                ? 'bg-black text-white'
                : 'bg-white text-black'
            }`}
          >
            ...
            <div>
              ...
              <div className="text-xs font-mono text-gray-400 mt-1">
                Total Identities:{' '}
                {selectedDeptStats.totalUsers}
                {' • '}
                Incidents:{' '}
                {selectedDeptStats.deptIncidents}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {selectedDeptStats.users.length > 0 ? (
                selectedDeptStats.users.map((user) => (
                  ...
                  >
                    <div>
                      <div className="font-serif-display text-sm font-medium">
                        {user.name}
                      </div>
